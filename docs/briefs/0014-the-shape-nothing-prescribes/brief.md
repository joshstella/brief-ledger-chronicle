...
| `b — the shared locator` | One implementation of the status-line locator in `tools/lib/`, read by `open-briefs.sh` and `list-briefs.sh`. The two disagree today by design — one walks structurally past frontmatter, the other searches the whole file — and `c` adds a third reader that gates. Resolves to the permissive semantics, which can widen what a reader finds but cannot fail a ledger that reads correctly today. |
