---
name: next-brief-phase
description: >-
  Continue a multi-phase brief after a phase completes. Use when the user asks for next-brief-phase.
---

Continue an in-progress brief: fold in what the completed phases actually taught, **re-plan the remaining sequence**, and branch the next phase. The companion to the `start-brief` skill — this is the step that keeps a plan from barrelling through its original order after a finding should have changed it.
...
1. **Find the ledger.**
   ...
   - - If none is in progress, say so and point to the `start-brief` skill.
...
4. **RE-PLAN the remaining phases — this is the point of the command.**
   - Re-read the remaining sequence against what the completed phases *found*, not against the original plan. Did a completed phase resolve an open decision in a way that reorders, splits, merges, or removes a later phase? (The canonical case: a phase that was provisional-pending-a-decision in the `start-brief` skill — its downstream sequence is now either confirmed or rewritten.)
   - If the remaining sequence still holds, say so explicitly. If it changed, present the revised remainder and *why* it changed.

5. **Refresh codebase context.** Re-read the repo `AGENTS.md` and the files the next phase will touch — the codebase has moved since the brief was initiated.
