---
name: blc-next-brief-phase
description: >-
  Continue a multi-phase brief after a phase completes. Use when the user asks for blc-next-brief-phase.
---

# blc-next-brief-phase

Continue an in-progress brief: fold in what the completed phases actually taught, **re-plan the remaining sequence**, and branch the next phase. The companion to `blc-start-brief` — this is the step that keeps a plan from barrelling through its original order after a finding should have changed it.

Usage: blc-next-brief-phase [brief path-or-name]
...
5. **Refresh codebase context.** Run `bash tools/orient.sh`, then re-read the repo `AGENTS.md` or `CLAUDE.md` and the files the next phase will touch — the codebase has moved since the brief was initiated, and this is the step that notices. `orient`'s freshness line is the cheapest way to catch a checkout that has fallen behind between phases, which is exactly when it happens.
